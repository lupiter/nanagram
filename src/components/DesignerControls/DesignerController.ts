...
export class DesignerController {
  toggleCell(state: DesignerState, row: number, col: number): DesignerState {
    return produce(state, (draft) => {
      const newValue =
        draft.grid[row][col] === CellState.FILLED
          ? CellState.EMPTY
          : CellState.FILLED;
  ...
  toggleAutoCheck(state: DesignerState): DesignerState {
    return produce(state, (draft) => {
      draft.autoCheckEnabled = !draft.autoCheckEnabled;
    });
  }
  ...
  hasFilledCells(state: DesignerState): boolean {
    return state.grid.some((row) =>
      row.some((cell) => cell === CellState.FILLED),
    );
  }
  ...
  getStatusInfo(state: DesignerState): {
    message: string;
    ...
    if (!this.hasFilledCells(state)) {
      return {
        message: "Draw your puzzle by clicking cells",
        variant: "info",
        difficulty: null,
      };
    }
    ...
    if (state.hasUniqueSolution === true) {
      const difficultyText = state.difficulty
        ? ` (Difficulty: ${String(state.difficulty)}/5)`
        : "";
      return {
        message: `Puzzle has a unique solution!${difficultyText}`,
        variant: "success",
    ...
    if (state.hasUniqueSolution === false) {
      return {
        message: "Puzzle does not have a unique solution",
        variant: "error",
        difficulty: null,
      };
    }
  ...
  loadDesign(state: DesignerState, design: SavedDesign): DesignerState {
    ...
    const grid: PuzzleSolutionData = design.solution.map((row) =>
      row.map((cell) =>
        (cell as number) === 1 ? CellState.FILLED : CellState.EMPTY,
      ),
    );
