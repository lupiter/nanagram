...
export class HintChecker {
  ...
  private couldBeHint(
    ...
    const spaceNeededAfter = this.minSpaceForHints(
      hints,
      hintIndex + 1,
      hints.length - 1,
    );
  ...
  check(cells: CellState[], hints: Hint[], answerCells: CellState[]): Hint[] {
    ...
    for (const seq of currentSequences) {
      ...
      for (let hintIndex = 0; hintIndex < newHints.length; hintIndex++) {
        ...
        if (
          this.couldBeHint(seq.start, seq.length, hintIndex, newHints, gridSize)
        ) {
      ...
      if (precedingBlocks.length === 0) {
        ...
        validIndices = possibleHintIndices.filter((hintIndex) => {
          if (hintIndex === 0) return true;
          const minStart =
            this.minSpaceForHints(newHints, 0, hintIndex - 1) + 1;
      ...
      if (validIndices.length === 1) {
        ...
        const noEmptyLeft =
          seq.start === 0 ||
          !cells.slice(0, seq.start).some((c) => c === CellState.EMPTY);
        const noEmptyRight =
          seq.start + seq.length >= gridSize ||
          !cells
            .slice(seq.start + seq.length)
            .some((c) => c === CellState.EMPTY);
        if (noEmptyLeft || noEmptyRight) {
          const pinnedHintIndex = precedingBlocks.length;
          if (
            pinnedHintIndex < newHints.length &&
            validIndices.includes(pinnedHintIndex)
          ) {
