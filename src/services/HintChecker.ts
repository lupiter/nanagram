...
export class HintChecker {
  ...
  check(cells: CellState[], hints: Hint[], answerCells: CellState[]): Hint[] {
    ...
    for (let i = 0; i < cells.length; i++) {
      if (cells[i] === CellState.FILLED && answerCells[i] !== CellState.FILLED) {
    ...
    for (let i = 0; i < answerCells.length; i++) {
      console.log(i, answerCells[i], cells[i], resolvedHints, currentHintSize, usedHint, sealed);
      if (answerCells[i] === CellState.FILLED) {
        currentHintSize++;
        usedHint = usedHint && cells[i] === CellState.FILLED && sealed;
      } else if ((answerCells[i] === CellState.CROSSED_OUT || answerCells[i] === CellState.EMPTY) && cells[i] === CellState.CROSSED_OUT) {
        ...
        if ((answerCells[i] === CellState.CROSSED_OUT || answerCells[i] === CellState.EMPTY) && currentHintSize > 0) {
