...
console.log(
  "🧩 Difficulty Analysis - Comparing Your Ratings vs New Algorithm\n",
);
console.log("The new algorithm analyzes:");
console.log(
  "  • First pass progress - How many cells solved in first iteration",
);
console.log("  • Total iterations - How many passes the solver needs");
console.log(
  "  • Initial forced cells - Cells immediately determinable from single lines",
);
...
for (const dir of puzzleDirs) {
  ...
  for (const file of files) {
    ...
    const diffStr =
      diff === 0 ? "=" : diff > 0 ? `+${String(diff)}` : String(diff);
...
console.log(
  `  Exact matches: ${String(matches)}/${String(results.length)} (${((matches / results.length) * 100).toFixed(0)}%)`,
);
console.log(
  `  Average absolute difference: ${(totalDiff / results.length).toFixed(2)}`,
);
...
const sorted = [...results].sort(
  (a, b) => Math.abs(b.algo - b.yours) - Math.abs(a.algo - a.yours),
);
const mostOff = sorted
  .filter((r) => Math.abs(r.algo - r.yours) >= 2)
  .slice(0, 5);
...
if (mostOff.length > 0) {
  ...
  for (const r of mostOff) {
    const diff = r.algo - r.yours;
    console.log(
      `    ${r.relativePath}: You=${String(r.yours)}, Algo=${String(r.algo)} (${diff > 0 ? "+" : ""}${String(diff)})`,
    );
  }
}
